...
## 1. Decompression numeric/index fix (modern2)
  ...
  - Affected file: `packages/msbwt-modern2/MUS/MultiStringBWT.py` (the only
  source line change of the fix: the `decompressBlocks` fill loop converts
  the `<u8` run-count scalar with `int(...)` before slice arithmetic; the
  modern2 file differs from the frozen original by exactly those three
  lines).
  ...
  file SHA-256
  `f536d60f356ed4a34e8253eecdd91e90db8c2ba3430c2e3d3e1855bb49007ef4`). The
  whole-file bytes differ from the compiled-builder golden
  ...
  byte changes.
  - Interoperability: the decompressed primary is a valid `msbwt.npy`
  loadable by the legacy byte reader semantics; the compressed input
  `comp_msbwt.npy` is untouched. Reader-derived indexes (`totalCounts.npy`,
  `fmIndex.npy` for byte; `totalCounts.p`, `comp_fmIndex.npy`,
  `comp_refIndex.npy` for compressed sources) are byte-identical to the
  legacy reader outputs for the same payloads.
...
## 2. `merge -p 1` CLI numProcs fix (modern2)
  ...
  - Old (frozen) behavior: `merge -p 1` raises `UnboundLocalError: local
  variable 'numProcs' referenced before assignment` because `numProcs` is
  bound only inside the `args.numProcesses > 1` branch. The output directory
  is created but empty and the inputs are unchanged. The committed failure
  evidence
  ...
  `dd91d69ee2785c88652790c8c2b892a173e4e1f57b703bfc00f1a2938dc1a15f`, `|u1`,
  shape `(48L,)`, payload
  ...
  byte-identical to the committed secondary regenerated-source oracle and to
  a clean-from-scratch combined construction.
  - Rationale: genuine CLI correctness defect, explicitly pre-authorized as
  a LEGACY SOURCE BUG fix by the resolved merge compatibility policy
  ...
  already binds. No merge mathematics, persistent format, or output file set
  changes.
  - Interoperability: the merged output is byte-identical to the committed
  secondary regenerated-source oracle and loadable by the legacy reader; the
  frozen `-p 1` failure evidence remains authoritative for the frozen CLI
  only. The historical committed `GenericMerge.c` SIGSEGV (LEGACY
  GENERATED-CODE DEFECT) is a separate preserved defect that modern2 never
  reproduces.
...
- Status: executed and validated — Windows port (133/133 tests pass).
...
- Old behavior: Cython code using `arr.shape` directly (where `arr` is a
  `cdef np.ndarray`) — on all platforms, the shape accessor returns a C
...
- Status: verified — 133/133 tests pass on native Windows.
...
p.dtype('a9') is invalid under NumPy 2.x -> TypeError);
    * compiled path computes mate offsets as i+1-2*j where the paired/
      unpaired test indexes the sequence string (r[1-j]) instead of the
...
- New (modern3) behavior: all four entry points raise
  ...
  core dependencies; no `bam` extra is advertised).  Ordinary FASTA/FASTQ
...
## M3-SOL-R1: Modern3 release-contract snapshot (documentation repair)

- Status: documentation only; no executable behavior is claimed by this
  entry.  The facts below restate the validated release contract recorded in
  `docs/modernization/MODERN3_RELEASE_NOTES.md` and `pyproject.toml` so this
  document cannot be read as contradicting it.
- Supported runtime: CPython >=3.14; NumPy >=2.5,<3 as the only runtime
  dependency (pysam is NOT a dependency).
- Supported platforms: Windows x86-64 and Linux x86-64 compiled wheels
  (`cp314-cp314-win_amd64`, `cp314-cp314-linux_x86_64`).  No macOS, ARM, or
  universal wheel exists or is claimed.
- BAM/SAM input: explicitly unsupported on every platform (see the M3-13C
  entry above).
- Persistence scope: the authoritative byte/RLE BWT, FM/interleave,
  provenance, source-metadata, read-provenance, tag, quality, LCP, and
  removal artifacts documented by the M3-12 persistence matrix are frozen;
  the Python-2 `totalCounts.p` reverse direction remains a derived,
  rebuildable one-way exception.
- Intentional feature exclusions carried over from enhanced-modern2:
  F8B and F11B/F11C are absent by design; no LZW encoder exists and LZW
  round-trip behavior carries no validation claim.
- Known numeric limitations: Multimerge does not support a single read
