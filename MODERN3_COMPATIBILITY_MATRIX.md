...
| `totalCounts.p` | pure-Python reader (side effect) | pure-Python reader | RESOLVED M3-5: m2→m3 PASS | Produced via `pickle.dump` of a numpy array (protocol 0 on py2). Verified empirically on Windows (M3-5): modern3 reads py2-written protocol-0 files with full semantic parity. Reverse direction fails as expected: py3 writes protocol 5 (`unsupported pickle protocol`) and even protocol-2 pickles reference `numpy._core.multiarray`, unloadable by py2-era numpy — a rebuildable derived cache, removed by `clearAuxiliaryData`; no migration planned. Note: both pure readers share a historical cold-start defect (first-time FM construction raises a float64-cast TypeError when `fmIndex.npy` is absent); parity-preserved, compiled readers unaffected. |
