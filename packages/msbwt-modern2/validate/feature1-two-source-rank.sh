#!/usr/bin/env bash
# msbwt-modern2 Feature 1 validation driver: two-source rank-aware queries.
#
# Validates the additive pure-Python layer MUS/SourceIndex.py over:
#   - the public merged BWT query API (findIndicesOfStr / countOccurrencesOfSeq)
#   - the retained GenericMerge provenance file inter0.npy
# using REAL modern2 construction (pp + cfpp) and REAL GenericMerge merges.
#
# Gates (all must pass):
#   1. environment: CPython 2.7, Cython 3.0.x, NumPy 1.16.6, pysam 0.15.4
#   2. all modern2 Python-2 tests green (includes the Feature-1 suite)
#   3. fixture hashes (uniform-a.fastq, uniform-b.fastq)
#   4. canonical Feature-1 integration: committed merged hashes, every
#      committed query + required query categories split by source equal to
#      the standalone source BWTs, dollar/absent/only-A/only-B/both queries
#   5. independent oracle: merged BWT == rotation-sort of the union; EVERY
#      unambiguous inter0.npy bit matches the per-rotation source prediction;
#      tied blocks match per-source multiplicity; per-source totals equal the
#      standalone BWT lengths
#   6. one merged FM search per source-aware query (instrumented real BWT)
#   7. givenRange legacy semantics preserved by the Feature-1 layer
#   8. rank cache identity: a same-length replaced inter0.npy must NOT be
#      served from the stale cache (rebuild instead)
#   9. deterministic edge corpus (12 source-pair classes)
#   10. randomized differential suite (50 source pairs, seed 20260811):
#       source0 == standalone A, source1 == standalone B,
#       source0 + source1 == legacy merged count, for every query
#   11. performance/memory sanity measurements
# If --evidence is given, a JSON record of every gate is written.
set -euo pipefail

usage() {
    printf '%s\n' 'Usage: validate/feature1-two-source-rank.sh --prefix /absolute/prefix --package /absolute/package --repository /absolute/repo [--evidence /absolute/out.json]'
}
...
WORK=$(mktemp -d /tmp/modern2-feature1-XXXXXX)
...
run_evidence() {
    # the evidence generator runs the canonical case, edge corpus, 50-case
    # randomized suite, oracle bit verification, cache probes, one-search
    # instrumentation, givenRange probe, and performance sanity; any
    # mismatch fails with a full dump.
    (cd "$PACKAGE" && "$PREFIX/bin/python" -B \
        validate/feature1_two_source_evidence.py "$WORK/evidence.json")
    python3 - "$WORK" <<'PY'
import json, os, sys
work = sys.argv[1]
data = json.load(open(os.path.join(work, 'evidence.json')))
assert data['mismatch_count'] == 0, data['mismatch_count']
assert data['randomized_source_pairs'] == 50, data['randomized_source_pairs']
assert data['seed'] == 20260811, data['seed']
assert data['query_comparisons'] > 5000, data['query_comparisons']
assert data['one_search_probe']['findIndicesOfStr_calls'] == 4, data['one_search_probe']
assert data['cache_same_length_replaced']['stale_cache_detected']
# the canonical interleave oracle check must have run with exact bit counts
canonical = [c for c in data['cases'] if c['case'] == 'canonical'][0]
assert canonical['interleave']['zeros'] == 24, canonical['interleave']
assert canonical['interleave']['ones'] == 24, canonical['interleave']
assert canonical['interleave']['exact_bits_checked'] > 0
...
gate_py2_tests
record py2_tests '{"status": "ok", "suite": "packages/msbwt-modern2 tests, including Feature-1"}'
gate_fixtures
...
record final '{"branch": "enhanced-modern2", "milestone": "enhanced-modern2-feature1-two-source-rank", "feature": "Feature 1: Two-Source Rank-Aware Queries", "status": "passed"}'
if [ -n "$EVIDENCE" ]; then cp "$RESULT_FILE" "$EVIDENCE"; printf 'Evidence written to %s\n' "$EVIDENCE"; fi
printf 'feature1-two-source-rank OK\n'
