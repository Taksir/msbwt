#!/usr/bin/env bash
# msbwt-modern2 Feature 12 validation driver: generic BWT-aligned tags.
#
# Validates the Feature-12 layer (MUS/BWTTags + Feature-12 additions to
# MUS/MultiSourceProvenance, MUS/MultiSourceQuery, MUS/SourceRemoval)
# over:
#   - the ten-source read-derived fixture with the strongest row-identity
#     tag: every merged row's tag equals the independently walked row
#     identity (191 rows); two-way and balanced 10-source merges;
#   - one-sided tags: rejection without a declared missing value BEFORE
#     the BWT merge; exact missing-row filling with a declared value;
#   - retrofit from leaf arrays equals the merge-produced tag
#     element-for-element with BWT bytes unchanged;
#   - Feature-10 removal: reduced tags == original[keep_mask]; single-
#     source output equals the original leaf array;
#   - query lifecycle: interval slices, source-filtered values, exact
#     value counts;
...
#   2. all modern2 Python-2 tests green (includes the Feature-12 suite)
...
usage() {
    printf '%s\n' 'Usage: validate/feature12-bwt-tags.sh --prefix /absolute/prefix --package /absolute/package --repository /absolute/repo [--evidence /absolute/out.json]'
}
...
WORK=$(mktemp -d /tmp/modern2-feature12-XXXXXX)
...
run_evidence() {
    (cd "$PACKAGE" && "$PREFIX/bin/python" -B \
        validate/feature12_bwt_tags_evidence.py "$WORK/evidence.json")
    python3 - "$WORK" <<'PY'
import json, os, sys
work = sys.argv[1]
...
assert merged['two_way_exact']
one = data['one_sided_policies']
assert one['without_missing'] and one['with_missing']
retrofit = data['retrofit']
assert retrofit['bwt_unchanged']
...
gate_py2_tests
record py2_tests '{"status": "ok", "suite": "packages/msbwt-modern2 tests, including Feature-12"}'
gate_fixtures
...
record final '{"branch": "enhanced-modern2", "milestone": "enhanced-modern2-feature12-bwt-tags", "feature": "Feature 12: Generic BWT-Aligned Tag Arrays", "status": "passed"}'
if [ -n "$EVIDENCE" ]; then cp "$RESULT_FILE" "$EVIDENCE"; printf 'Evidence written to %s\n' "$EVIDENCE"; fi
printf 'feature12-bwt-tags OK\n'
