#!/usr/bin/env bash
# msbwt-modern2 Feature 13A validation driver: whole-index benchmark
# harness and backend contract.
#
# Validates MUS/Benchmarking and MUS/BackendContract over:
#   - a fully layered 10-source package (provenance + read provenance +
#     tags + Q1 quality + Feature-11A LCP): run metrics and disk
#     accounting equal independent oracles; Q1 quality payload
#     single-counted; LCP accounting exact;
#   - the Feature-10 reduced package (133 rows);
#   - backend contract (search-core tier satisfied by the legacy
#     adapter; framework contract covers Features 1-12/Q1/11A);
#   - query benchmark and build-operation hook; comparison table.
#
# Gates (all must pass):
#   1. environment: CPython 2.7, Cython 3.0.x, NumPy 1.16.6, pysam 0.15.4
#   2. all modern2 Python-2 tests green (includes the Feature-13A suite)
...
usage() {
    printf '%s\n' 'Usage: validate/feature13a-benchmark.sh --prefix /absolute/prefix --package /absolute/package --repository /absolute/repo [--evidence /absolute/out.json]'
}
...
WORK=$(mktemp -d /tmp/modern2-feature13a-XXXXXX)
...
run_evidence() {
    (cd "$PACKAGE" && "$PREFIX/bin/python" -B \
        validate/feature13a_benchmark_evidence.py "$WORK/evidence.json")
    python3 - "$WORK" <<'PY'
import json, os, sys
work = sys.argv[1]
...
assert structural['quality_single_counted']
assert structural['lcp_boundaries'] == 190
assert structural['naive_u32_rle_smaller'] is False
reduced = data['reduced']
assert reduced['N'] == 133
assert reduced['filesystem_oracle_exact']
assert reduced['run_oracle_exact']
contract = data['backend_contract']
assert contract['contract_version'] == 1
assert contract['search_core_satisfied']
assert contract['framework_all_features']
query = data['query_benchmark']
assert query['operations'] >= 8
assert query['repeats'] == 3
comparison = data['comparison']
assert comparison['ten_N'] == 191
assert comparison['seven_N'] == 133
...
gate_py2_tests
record py2_tests '{"status": "ok", "suite": "packages/msbwt-modern2 tests, including Feature-13A"}'
gate_fixtures
...
record final '{"branch": "enhanced-modern2", "milestone": "enhanced-modern2-feature13a-benchmark", "feature": "Feature 13A: Whole-Index Benchmark Harness + Backend Contract", "status": "passed"}'
if [ -n "$EVIDENCE" ]; then cp "$RESULT_FILE" "$EVIDENCE"; printf 'Evidence written to %s\n' "$EVIDENCE"; fi
printf 'feature13a-benchmark OK\n'
