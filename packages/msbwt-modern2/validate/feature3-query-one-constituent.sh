#!/usr/bin/env bash
# msbwt-modern2 Feature 3 validation driver: query one constituent inside a
# multi-merged MSBWT.
#
# Validates the additive pure-Python layer MUS/MultiSourceQuery.py over:
#   - the ten-source read-derived fixture (>=350 patterns x 10 sources)
#     against independent standalone suffix-order oracles
#   - an independent naive interval-projection oracle (direct bit-array
#     counting through unpack_interleave, no sampled-rank code shared)
#   - one merged FM search per constituent query, lazy rank-node loading
#   - rank-cache round trip + reload and hardening probes (same-length
#     replaced interleave, corrupt cache, wrong stride, legacy cache)
#   - REAL modern2 construction (pp + cfpp) and REAL GenericMerge merges:
#     canonical two-source, 5-source balanced, 4-source unbalanced chain,
#     givenRange delegation, one-search instrumentation
...
#   2. all modern2 Python-2 tests green (includes the Feature-3 suite)
#   3. fixture hashes (uniform-a.fastq, uniform-b.fastq)
#   4. evidence generator: ten-source fixture, naive oracle, hardening
#      probes, real integrations, randomized differential, zero mismatches
# If --evidence is given, a JSON record of every gate is written.
set -euo pipefail

usage() {
    printf '%s\n' 'Usage: validate/feature3-query-one-constituent.sh --prefix /absolute/prefix --package /absolute/package --repository /absolute/repo [--evidence /absolute/out.json]'
}
...
WORK=$(mktemp -d /tmp/modern2-feature3-XXXXXX)
...
run_evidence() {
    (cd "$PACKAGE" && "$PREFIX/bin/python" -B \
        validate/feature3_query_one_constituent_evidence.py "$WORK/evidence.json")
    python3 - "$WORK" <<'PY'
import json, os, sys
work = sys.argv[1]
...
assert ten['interval_comparisons'] >= 3500
assert ten['mismatches'] == 0
assert ten['one_search_probe']
assert ten['lazy_loading_probe']
assert ten['rank_cache_round_trip']
for probe, detected in data['cache_hardening_probes'].items():
...
assert real['four_source_unbalanced_chain']['mismatches'] == 0
assert real['given_range_delegation']
assert real['one_search_per_query_real']
rand = data['randomized']
assert rand['seed'] == 20260811
assert rand['datasets'] == 6
assert rand['mismatches'] == 0
assert data['performance']['rank_nodes_touched'] > 0
...
gate_py2_tests
record py2_tests '{"status": "ok", "suite": "packages/msbwt-modern2 tests, including Feature-3"}'
gate_fixtures
...
record final '{"branch": "enhanced-modern2", "milestone": "enhanced-modern2-feature3-query-one-constituent", "feature": "Feature 3: Query One Constituent Inside a Multi-Merged MSBWT", "status": "passed"}'
if [ -n "$EVIDENCE" ]; then cp "$RESULT_FILE" "$EVIDENCE"; printf 'Evidence written to %s\n' "$EVIDENCE"; fi
printf 'feature3-query-one-constituent OK\n'
