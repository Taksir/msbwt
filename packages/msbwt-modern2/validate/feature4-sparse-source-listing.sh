#!/usr/bin/env bash
# msbwt-modern2 Feature 4 validation driver: sparse nonzero-source listing
# with exact counts.
#
# Validates the Feature-4 layer (MUS/MultiSourceQuery.nonzeroSources /
# listSourcesWithOccurrences / nonzero_sources_interval) over:
#   - the ten-source read-derived fixture: sparse listing == Point-3
#     per-source count oracle filtered to positives, exact
#     constituent-local intervals, sum-of-counts == merged count,
...
#   - pruning statistics vs an independent recursive DFS counter;
#     zero/one/few/all-sources categories
#   - REAL modern2 construction (pp + cfpp) and REAL GenericMerge merges:
#     canonical two-source, 5-source balanced, givenRange delegation
...
#   2. all modern2 Python-2 tests green (includes the Feature-4 suite)
#   3. fixture hashes (uniform-a.fastq, uniform-b.fastq)
#   4. evidence generator: sparse oracle equality, pruning stats, real
#      integrations, randomized differential, zero mismatches
# If --evidence is given, a JSON record of every gate is written.
set -euo pipefail

usage() {
    printf '%s\n' 'Usage: validate/feature4-sparse-source-listing.sh --prefix /absolute/prefix --package /absolute/package --repository /absolute/repo [--evidence /absolute/out.json]'
}
...
WORK=$(mktemp -d /tmp/modern2-feature4-XXXXXX)
...
run_evidence() {
    (cd "$PACKAGE" && "$PREFIX/bin/python" -B \
        validate/feature4_sparse_source_evidence.py "$WORK/evidence.json")
    python3 - "$WORK" <<'PY'
import json, os, sys
work = sys.argv[1]
...
assert ten['sparse_comparisons'] >= 350
assert ten['interval_comparisons'] >= 250
assert ten['mismatches'] == 0
assert ten['one_search_probe']
assert ten['pruning_probe']
assert ten['absent_patterns'] > 0
assert ten['one_source_patterns'] > 0
assert ten['average_internal_nodes_nonempty'] > 0
for name, value in data['pruning_categories'].items():
    assert value, name
...
assert data['performance']['sparse_queries'] == 40
...
gate_py2_tests
record py2_tests '{"status": "ok", "suite": "packages/msbwt-modern2 tests, including Feature-4"}'
gate_fixtures
...
record final '{"branch": "enhanced-modern2", "milestone": "enhanced-modern2-feature4-sparse-source-listing", "feature": "Feature 4: Sparse Nonzero-Source Listing With Exact Counts", "status": "passed"}'
if [ -n "$EVIDENCE" ]; then cp "$RESULT_FILE" "$EVIDENCE"; printf 'Evidence written to %s\n' "$EVIDENCE"; fi
printf 'feature4-sparse-source-listing OK\n'
