...
usage() {
    printf '%s\n' 'Usage: validate/feature7-subset-groups.sh --prefix /absolute/prefix --package /absolute/package --repository /absolute/repo [--evidence /absolute/out.json]'
}
...
WORK=$(mktemp -d /tmp/modern2-feature7-XXXXXX)
...
run_evidence() {
    (cd "$PACKAGE" && "$PREFIX/bin/python" -B \
        validate/feature7_subset_group_evidence.py "$WORK/evidence.json")
    python3 - "$WORK" <<'PY'
import json, os, sys
work = sys.argv[1]
...
assert ten['subset_comparisons'] >= 2000
assert ten['mismatches'] == 0
assert ten['one_search_probe']
assert ten['subtree_shortcut_probe']
assert ten['metadata_isolation_probe']
for name, detected in data['validation_probes'].items():
...
assert data['performance']['subset_queries'] == 20
...
gate_py2_tests
record py2_tests '{"status": "ok", "suite": "packages/msbwt-modern2 tests, including Feature-7"}'
gate_fixtures
...
record final '{"branch": "enhanced-modern2", "milestone": "enhanced-modern2-feature7-subset-groups", "feature": "Feature 7: Source Subsets / Metadata Groups / Predicates", "status": "passed"}'
if [ -n "$EVIDENCE" ]; then cp "$RESULT_FILE" "$EVIDENCE"; printf 'Evidence written to %s\n' "$EVIDENCE"; fi
printf 'feature7-subset-groups OK\n'
