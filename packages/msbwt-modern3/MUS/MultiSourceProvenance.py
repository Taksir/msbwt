...
def _atomic_replace(source_path, dest_path):
    ...
    Python 3 uses ``os.replace`` when available.
    """
...
def _atomic_write_json(path, manifest):
    ...
    bytes) and written in binary mode.
    """
...
def _call_merge(merge_two_func, left_dir, right_dir, output_dir, num_procs,
    ...
    ``mergeTwoMSBWTs(input1, input2, merged, numProcs, logger)``.  Try
    keywords first (the documented call shape for test adapters), then fall
    back to positional on an argument-signature TypeError only.
    """
    try:
...
def holt_merge_two(left_dir, right_dir, output_dir, num_procs=1, logger=None):
    ...
    bytes: under Python 3 the paths are ``os.fsencode``d.
    """
    if logger is None:
        logger = logging.getLogger(__name__)
