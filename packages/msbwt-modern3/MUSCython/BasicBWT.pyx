...
cdef class BasicBWT(object):
    ...
    cpdef tuple findIndicesOfStr(BasicBWT self, object seq, tuple givenRange=None):
        '''
    ...
    cpdef list findIndicesOfRegex(BasicBWT self, object seq, tuple givenRange=None):
        '''
        This function will search for a string and find the location of that string OR the last index less than it. It also
    ...
    cpdef list findStrWithError(BasicBWT self, object seq, object bonusStr):
        ...
        cdef bytes _seq_b = seq if isinstance(seq, bytes) else seq.encode('ascii')
        cdef unsigned char * seq_view = _seq_b
        cdef bytes _bonusStr_b = bonusStr if isinstance(bonusStr, bytes) else bonusStr.encode('ascii')
        cdef unsigned char * bonusStr_view = _bonusStr_b
    ...
    cpdef list findKmerWithErrors(BasicBWT self, object seq, np.uint64_t editDistance, np.uint64_t minThresh=1):
        ...
        while stackIndex >= 0:
            ...
            #print ''.join(reversed([chr(self.numToChar_view[currSymStack_view[x]]) if currTypeStack_view[x] != 2 else '-' for x in range(0, stackIndex+1)]))
            #print ''.join(reversed([str(currTypeStack_view[x]) for x in range(0, stackIndex+1)]))
                        ...
                        if currSeqIndex == 0:
                            ...
                            if currEditStack_view[stackIndex] == 0:
                                ...
                                ret.append((lowArrayStack_view[stackIndex][c], highArrayStack_view[stackIndex][c], 
                                            ''.join(reversed([chr(self.numToChar_view[currSymStack_view[x]]) if currTypeStack_view[x] != 2 else '' for x in range(0, stackIndex+1)])),
                                            ''.join(reversed([str(currTypeStack_view[x]) for x in range(0, stackIndex+1)]))))
