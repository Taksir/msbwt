...
from . import MSBWTCompGenCython as MSBWTCompGen
...
def createMSBWTFromSeqs(seqArray, mergedDir, numProcs, areUniform, logger):
    ...
    seqCopy = np.frombuffer(seqCopy.encode('ascii'), dtype='<u1').copy()
...
def preprocessBams(bamFNs, outputDir, areUniform, logger):
    ...
    for fnID, fn in enumerate(bamFNs):
        ...
        nr = next(bamFile)
        constantSize = len(nr.query_sequence)

        #go through each line
        while nr != None:
            #collect all reads that are the same as nr
            aligns = []
            nqname = nr.query_name
            while nr != None and nr.query_name == nqname:
                aligns.append(nr)
                try:
                    nr = next(bamFile)
                except:
            ...
            reads = [None, None]
            for a in aligns:
                if len(a.query_sequence) != constantSize:
                    print('DIFF SIZE='+str(len(a.query_sequence)))
                
                if a.flag & REVERSE_COMPLEMENTED_FLAG == 0:
                    #not reverse complemented
                    seq = a.query_sequence
                else:
                    seq = reverseComplement(a.query_sequence)
