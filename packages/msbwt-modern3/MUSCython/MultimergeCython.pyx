...
def mergeTwoMSBWTs(str inputMsbwtDir1, str inputMsbwtDir2, str mergedDir, unsigned long numProcs, logger):
    ...
    for y in range(bwtLen1//8+1, (bwtLen1+bwtLen2)//8+1):
        inter0_view[y] = 0xFF
        inter1_view[y] = 0xFF
...
def mergeUsingInterleave(str inputMsbwtDir1, str inputMsbwtDir2, str mergedDir, str interleaveFN):
    '''
...
def levelIterator(offsets, bint areUniform, mergedDir, unsigned long currMultiplier, unsigned long splitDist,
    ...
    if areUniform:
        ...
        for x in range(0, numSeqs, currMultiplier):
...
def buildViaMerge256(tup):
    ...
    if numInputs == 2:
        ...
        with nogil:
            #initialize the first half to all 0s, 0x00
            for y in range(0, (offsets_view[1]-offsets_view[0])//8):
                inter0_view[y] = 0x00
                inter1_view[y] = 0x00
            ...
            #all remaining bytes are all ones, 0xFF
            for y in range((offsets_view[1]-offsets_view[0])//8+1, (offsets_view[2]-offsets_view[0])//8+1):
                inter0_view[y] = 0xFF
                inter1_view[y] = 0xFF
        ...
        for x in range(0, numInputs):
            for y in range(offsets_view[x]-offsets_view[0], offsets_view[x+1]-offsets_view[0]):
                inter0_view[y] = x
    ...
    while changesMade:
        ...
        #with nogil:
        for x in range(0, offsetsCopy.shape[0]):
    ...
    if numInputs == 2:
        for x in range(0, offsets_view[numInputs]-offsets_view[0]):
            #get the read, the symbol, and increment the position in that read
            if getBit_p(inter0_p, x):
    ...
    for x in range(offsets_view[0], offsets_view[numInputs]):
...
cdef inline tuple targetedIterationMerge2(np.uint8_t * seqs0_view, np.uint8_t * seqs1_view, 
        ...
        for y in range(0, ranges.shape[0]):
            ...
            if startIndex0 == ranges_view[y,0] and startIndex1 == ranges_view[y,1] and y != 0:
                ...
                if (startIndex0 >> binBits) == (ranges_view[y,0] >> binBits):
                    for x in range(startIndex0, ranges_view[y,0]):
                ...
                if (startIndex1 >> binBits) == (ranges_view[y,1] >> binBits):
                    for x in range(startIndex1, ranges_view[y,1]):
    ...
    if collapseEntries:
        for currIndex in range(1, exIndex):
    ...
    for z in range(0, nvc):
        for y in range(0, neIndex_view[z]):
            ...
            for x in range(totalStart//8, (totalStart+dist)//8):
