...
Resolved policy (see ``MERGE_INDEXING_PLAN.md``): the deterministic segfault
of the committed historical generated C ``MUSCython/GenericMerge.c`` is a
LEGACY GENERATED-CODE DEFECT and is preserved as historical failure evidence.
Successful merge semantics are established through a narrowly scoped SECONDARY
REGENERATED-SOURCE ORACLE: the unchanged frozen ``GenericMerge.pyx`` is
regenerated with the verified profile's pinned Cython 0.29.36 in the disposable
built source copy, and only the ``MUSCython.GenericMerge`` extension is
rebuilt.  No frozen file is modified.  The runner verifies the frozen 40-file
projection (including the exact ``.pyx`` bytes) before regeneration, records
the committed ``GenericMerge.c`` hash as historical evidence, captures one
``merge -p 2`` segfault probe per canonical run on the committed build, and
then runs the canonical merge twice on the regenerated build.
...
Canonical case (``merge-uniform-a-uniform-b``):
  ...
  MERGE   ``merge -p 2 MERGED IN_A IN_B`` (regenerated merge extension)

The frozen ``merge`` CLI requires ``-p N`` with ``N > 1`` because the default
``-p 1`` path raises ``UnboundLocalError: local variable 'numProcs' referenced
before assignment`` (a LEGACY SOURCE BUG recorded separately); the canonical
invocation ``merge -p 2`` clamps to a single process and is deterministic.
...
FROZEN_MANIFEST_NAME = "frozen-source.sha256"
...
def verify_frozen_projection(built_source, environment_root):
    """Prove the built scratch copy is still the exact frozen projection.

    The full 40-file projection was verified by ``probe_legacy`` at the source
    level.  Here the scratch copy is re-verified with the two known-volatile
    exemptions: ``*.pyc`` destinations are regenerated by Python at import
    time, and the historical ``README.md`` mapping exists at its destination
    path.  The two regeneration inputs (``GenericMerge.pyx`` and the committed
    ``GenericMerge.c``) are hash-checked strictly against the pinned manifest.
    """
    import verify_frozen_source
    manifest = os.path.join(environment_root, FROZEN_MANIFEST_NAME)
    mismatches = []
    checked = 0
    for expected, destination_relative, source_relative in verify_frozen_source.read_manifest(manifest):
        if destination_relative.endswith(".pyc"):
            continue
        if destination_relative == "README.md":
            path = os.path.join(built_source, destination_relative)
        else:
            path = os.path.join(built_source, *source_relative.split("/"))
        if not os.path.isfile(path):
            mismatches.append((destination_relative, "missing", expected, None))
            continue
        actual = verify_frozen_source.sha256_file(path)
        checked += 1
        if actual != expected:
            mismatches.append((destination_relative, "sha256", expected, actual))
    if mismatches:
        raise RuntimeError("frozen projection verification failed in merge milestone: {0}".format(
            repr(mismatches)))
    pyx_path = os.path.join(built_source, GENERIC_MERGE_PYX)
    c_path = os.path.join(built_source, GENERIC_MERGE_C)
    return {
        "manifest": os.path.basename(manifest),
        "checked_files": checked,
        "pyc_files_exempted": True,
        "mismatches": [],
        "pyx_sha256": sha256_file(pyx_path),
        "committed_generated_c_sha256": sha256_file(c_path),
    }


def segfault_probe(recorder, built_source, operation_root, label, input_a, input_b):
    """Capture the historical committed generated-C segfault on disposable copies.

    This runs on the UNREGENERATED build and records the exact legacy
    generated-code defect: ``merge -p 2`` exits -11 (SIGSEGV), stderr is
    empty, no output files are produced, and the input copies gain exactly
    ``totalCounts.npy`` and ``fmIndex.npy`` before the crash.
    """
    probe_root = os.path.join(operation_root, "segfault-p2")
    require_new(probe_root, "segfault probe root")
    os.makedirs(probe_root)
    in_a = os.path.join(probe_root, "input-a")
    in_b = os.path.join(probe_root, "input-b")
    copy_tree(input_a, in_a)
    ...
    after_a = inventory(in_a)
    after_b = inventory(in_b)
    if returncode != SEGFAULT_EXIT_CODE:
        raise RuntimeError("merge -p 2 segfault probe exited {0}, expected -11".format(returncode))
    if streams["stderr_sha256"] != hashlib.sha256(b"").hexdigest():
        raise RuntimeError("merge -p 2 segfault probe produced stderr: {0}".format(
            streams["stderr_sha256"]))
    output_files = sorted(os.listdir(out)) if os.path.isdir(out) else None
    if output_files != []:
        raise RuntimeError("merge -p 2 segfault probe produced output files: {0}".format(output_files))
    ...
    if a_side_effects != ["fmIndex.npy", "totalCounts.npy"] or \
            b_side_effects != ["fmIndex.npy", "totalCounts.npy"]:
        raise RuntimeError("merge segfault probe input side effects differ: {0} {1}".format(
            a_side_effects, b_side_effects))
    return {
...
def regenerate_generic_merge(recorder, built_source, environment_root, config):
    """Regenerate and rebuild only the merge extension from the frozen .pyx.

    Secondary regenerated-source oracle procedure (pinned): verify the exact
    frozen ``GenericMerge.pyx`` bytes and the committed ``GenericMerge.c``
    bytes against the frozen manifest, then force-cythonize the ``.pyx`` with
    the profile's pinned Cython and rebuild ``MUSCython.GenericMerge`` in
    place.  No frozen file is modified and no other extension is rebuilt.
    """
    import verify_frozen_source
    regen = config["regeneration"]
    pyx_path = os.path.join(built_source, GENERIC_MERGE_PYX)
    c_path = os.path.join(built_source, GENERIC_MERGE_C)
    pre_pyx_hash = sha256_file(pyx_path)
    pre_c_hash = sha256_file(c_path)
    if pre_pyx_hash != regen["frozen_pyx_sha256"]:
        raise RuntimeError("frozen GenericMerge.pyx hash differs from pinned contract")
    if pre_c_hash != regen["committed_generated_c_sha256"]:
        raise RuntimeError("committed GenericMerge.c hash differs from pinned contract")
    version_argv = [sys.executable, "-c", "import Cython; print(Cython.__version__)"]
    cythonize_argv = [sys.executable, "-c", regen["cythonize_source"]]
    build_argv = [sys.executable] + regen["build_argv"]
    run_required(recorder, "merge-cython-version", version_argv, built_source)
    version_record = recorder.records[-1]
    version_text = read_recorded_stream(recorder, version_record, "stdout").decode(
        "utf-8", "replace").strip()
    if version_text != regen["cython_version"]:
        raise RuntimeError("merge regeneration Cython version differs: {0!r}".format(version_text))
    run_required(recorder, "merge-cythonize-generic-merge", cythonize_argv, built_source)
    run_required(recorder, "merge-rebuild-generic-merge", build_argv, built_source)
    post_pyx_hash = sha256_file(pyx_path)
    post_c_hash = sha256_file(c_path)
    if post_pyx_hash != pre_pyx_hash:
        raise RuntimeError("merge regeneration modified the frozen .pyx")
    if post_c_hash == pre_c_hash:
        raise RuntimeError("merge regeneration did not replace the generated C")
    return {
        "oracle_class": regen["oracle_class"],
        "cython_version": version_text,
        "pyx_sha256_before": pre_pyx_hash,
        "pyx_sha256_after": post_pyx_hash,
        "pyx_unchanged": pre_pyx_hash == post_pyx_hash,
        "committed_generated_c_sha256": pre_c_hash,
        "regenerated_c_sha256": post_c_hash,
        "regenerated_c_different_from_committed": post_c_hash != pre_c_hash,
        "cythonize_source": regen["cythonize_source"],
        "build_argv": regen["build_argv"],
        "frozen_manifest": os.path.basename(os.path.join(
            environment_root, FROZEN_MANIFEST_NAME)),
    }
...
def run_cli_query(recorder, built_source, operation_root, label, merged_copy):
    ...
    # The query CLI prints logger lines to stdout before the result; the result
    # is the final non-empty line(s).
    count_lines = [line.strip() for line in count_stdout.decode("utf-8", "replace").splitlines()
                   if line.strip()]
    count_text = count_lines[-1] if count_lines else ""
    if count_text != "3":
...
def assert_determinism(result, config):
    ...
    for key in ("segfault",):
        ...
        comparisons[key] = {
            "exit_code_identical": left["exit_code"] == right["exit_code"],
            "signal_identical": left["signal"] == right["signal"],
            "stderr_sha256_identical": left["streams"]["stderr_sha256"] == right["streams"]["stderr_sha256"],
            "output_files_identical": left["output_files"] == right["output_files"],
            "input_a_side_effects_identical": left["input_a_side_effects"] == right["input_a_side_effects"],
            "input_b_side_effects_identical": left["input_b_side_effects"] == right["input_b_side_effects"],
...
def execute(recorder, built_source, run_dir, fixture_root, golden_root, manifest_path):
    ...
    try:
        result["frozen_projection"] = verify_frozen_projection(built_source, environment_root)
        run_roots = {}
        ...
        # Historical committed generated-C failure evidence, captured on the
        # unregenerated build before the secondary-oracle regeneration.
        for run_id in config["canonical_runs"]:
            run_root = run_roots[run_id]
            result["cases"][run_id]["segfault"] = segfault_probe(
                recorder, built_source, run_root, run_id + "-segfault",
                result["cases"][run_id]["input_a"]["path"],
                result["cases"][run_id]["input_b"]["path"])
        # Secondary regenerated-source oracle: regenerate and rebuild only the
        # merge extension from the exact frozen .pyx with pinned Cython.
        result["regeneration"] = regenerate_generic_merge(
            recorder, built_source, environment_root, config)
        # Canonical merges on the regenerated build.
        for run_id in config["canonical_runs"]:
            run_root = run_roots[run_id]
            merged = merge_case(
            ...
            result["cases"][run_id]["merged"] = merged
        # Reader evidence on disposable copies of each run's merged output.
        for run_id in config["canonical_runs"]:
            run_root = run_roots[run_id]
            merged = result["cases"][run_id]["merged"]
            result["cases"][run_id]["reader"] = reader_evidence(
                recorder, environment_root, built_source, raw_root, run_id,
                fixture_root, config, merged["output_path"])
