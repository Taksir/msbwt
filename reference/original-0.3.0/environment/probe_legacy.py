...
DEFAULT_MERGE_CASES = os.path.join(SCRIPT_DIR, "merge-milestone1-cases.json")
...
def run_route(
    ...
    if compression_options:
        if route != "pyx-historical-cython":
            raise RuntimeError("compression milestone requires the historical-pyx route")
        if any(returncode != 0 for returncode in gate.values()):
            route_result["compression_milestone1"] = {"status": "skipped-gate-failure"}
        else:
            import compression_milestone1
            route_result["compression_milestone1"] = compression_milestone1.execute(
                recorder, destination, run_dir, compression_options["fixture_root"],
                compression_options["golden_root"], compression_options["manifest"])
    if compression2_options:
        if route != "pyx-historical-cython":
            raise RuntimeError("compression recovery milestone requires the historical-pyx route")
        if any(returncode != 0 for returncode in gate.values()):
            route_result["compression_milestone2"] = {"status": "skipped-gate-failure"}
        else:
    ...
    if merge_options:
        if route != "pyx-historical-cython":
            raise RuntimeError("merge milestone requires the historical-pyx route")
        if any(returncode != 0 for returncode in gate.values()):
            route_result["merge_milestone1"] = {"status": "skipped-gate-failure"}
        else:
            import merge_milestone1
            route_result["merge_milestone1"] = merge_milestone1.execute(
                recorder, destination, run_dir, merge_options["fixture_root"],
                merge_options["golden_root"], merge_options["manifest"])
    return route_result
...
def parse_args(argv):
    ...
    parser.add_argument(
        "--compression-milestone1", action="store_true",
        help="run the manifest-driven clean RLE milestone after all route gates pass"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--merge-milestone1", action="store_true",
        help="run the manifest-driven canonical merge/reader-indexing milestone after all route gates pass"
    )
    ...
    parser.add_argument("--merge-manifest", default=DEFAULT_MERGE_CASES)
...
def main(argv=None):
    ...
    if args.compression_milestone1:
        if args.route != "pyx-historical-cython":
            raise SystemExit("--compression-milestone1 requires --route pyx-historical-cython")
        if not args.install_dependencies:
            raise SystemExit("--compression-milestone1 requires --install-dependencies")
        if not args.fixture_root or not args.golden_root:
            raise SystemExit("--compression-milestone1 requires --fixture-root and --golden-root")
        if sys.version_info[:2] != (2, 7) or platform.python_implementation() != "CPython":
            raise SystemExit("--compression-milestone1 requires CPython 2.7")
        compression_options = {
            "fixture_root": os.path.abspath(args.fixture_root),
            "golden_root": os.path.abspath(args.golden_root),
            "manifest": os.path.abspath(args.compression_manifest)
        }
    compression2_options = None
    if args.compression_milestone2:
        if args.route != "pyx-historical-cython":
            raise SystemExit("--compression-milestone2 requires --route pyx-historical-cython")
        if not args.install_dependencies:
            raise SystemExit("--compression-milestone2 requires --install-dependencies")
        if not args.fixture_root or not args.golden_root:
            raise SystemExit("--compression-milestone2 requires --fixture-root and --golden-root")
        if sys.version_info[:2] != (2, 7) or platform.python_implementation() != "CPython":
            raise SystemExit("--compression-milestone2 requires CPython 2.7")
        compression2_options = {
            "fixture_root": os.path.abspath(args.fixture_root),
            "golden_root": os.path.abspath(args.golden_root),
    ...
    merge_options = None
    if args.merge_milestone1:
        if args.route != "pyx-historical-cython":
            raise SystemExit("--merge-milestone1 requires --route pyx-historical-cython")
        if not args.install_dependencies:
            raise SystemExit("--merge-milestone1 requires --install-dependencies")
        if not args.fixture_root or not args.golden_root:
            raise SystemExit("--merge-milestone1 requires --fixture-root and --golden-root")
        if sys.version_info[:2] != (2, 7) or platform.python_implementation() != "CPython":
            raise SystemExit("--merge-milestone1 requires CPython 2.7")
        merge_options = {
            "fixture_root": os.path.abspath(args.fixture_root),
            "golden_root": os.path.abspath(args.golden_root),
            "manifest": os.path.abspath(args.merge_manifest)
        }
